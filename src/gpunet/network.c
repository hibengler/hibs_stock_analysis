  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        net->layer[l].upper_lower_weight[nu * (net->layer[l - 1].no_of_neurons+1)
	                                 +nl] =            2.0 * range * ((float) random() / RAND_MAX - 0.5);
      }
    }
  }
}
...
void net_reset_deltas(network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        net->layer[l].upper_lower_delta[nu * (net->layer[l - 1].no_of_neurons+1)
	                                 +nl]  = 0.0;
      }
    }
  }
...
void net_use_bias(network_t *net, int flag)
{
  ...
  if (flag != 0) {
    ...
    for (l = 0; l < net->no_of_layers; l++) {
      net->layer[l].neuron_output[net->layer[l].no_of_neurons] = 1.0;
    }
  } else {
    /* permanently set output of bias neurons to 0 */
    for (l = 0; l < net->no_of_layers; l++) {
      net->layer[l].neuron_output[net->layer[l].no_of_neurons] = 0.0;
    }
  }
}
...
static void allocate_layer(layer_t *layer, int no_of_neurons)
{
  ...
  layer->neuron_output = (float *)
        calloc(no_of_neurons + 1, sizeof(float));
  
  layer->neuron_learning_rate = (float *)
        malloc(no_of_neurons + 1* sizeof(float));
  /* set all learning rates to 1 */
  {int i; for (i=1;i<no_of_neurons;i++) { layer->neuron_learning_rate[i]=1.;}}
  
  layer->neuron_error = (float *)
        calloc(no_of_neurons + 1, sizeof(float));
...
static void allocate_weights(layer_t *lower, layer_t *upper)
{
  ...
  assert(lower != NULL);
  assert(upper != NULL);
  upper->upper_lower_weight =  (float *)calloc(
     (lower->no_of_neurons + 1)*upper->no_of_neurons, sizeof(float));
  upper->upper_lower_delta =  (float *)calloc(
     (lower->no_of_neurons + 1)*upper->no_of_neurons, sizeof(float));
...
void net_free(network_t *net)
{
  ...
  for (l = 0; l < net->no_of_layers; l++) {
    layer_t *layer;
    layer = net->layer +l;
    free(layer->upper_lower_delta);
    free(layer->upper_lower_weight);
    free(layer->neuron_error);
    free(layer->neuron_output);
    free(layer->neuron_learning_rate);
  }
  free(net->layer);
  free(net);
}
...
void net_set_weight(network_t *net, int l, int nl, int nu, float weight)
{
  ...
  net->layer[l].upper_lower_weight[nu* (net->layer[l].no_of_neurons+1)
                                    + nl] = weight;
}
...
float net_get_weight(const network_t *net, int l, int nl, int nu)
{
  ...
  return net->layer[l].upper_lower_weight[nu* (net->layer[l].no_of_neurons+1)
                                    + nl];
}
 ...
 * \Note - the neuron specific learning rate is not saved.
 */
int net_fprint(FILE *file, const network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        result =
            fprintf(file, "%f\n", net->layer[l].upper_lower_weight
	      [nu * ( net->layer[l - 1].no_of_neurons +1) +nl]);
        if (result < 0) {
          return result;
        }
      }
...
int net_fbprint(FILE *file, const network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      fwrite(&net->layer[l].upper_lower_weight
         [nu * ( net->layer[l - 1].no_of_neurons + 1)]
     , sizeof(float),
...
network_t *net_fbscan(FILE *file)
{
  ...
  layerlist = (int *)calloc(no_of_layers, sizeof(int));
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      fread(&net->layer[l].upper_lower_weight
         [nu * ( net->layer[l - 1].no_of_neurons + 1)]
     , sizeof(float),
             net->layer[l - 1].no_of_neurons + 1, file);
    }
...
static INLINE void set_input(network_t *net, const float *input)
{
  ...
  for (n = 0; n < net->input_layer->no_of_neurons; n++) {
    net->input_layer->neuron_output[n] = input[n];
  }
}
...
{
  return (float)(sigma_x * (1.0f - sigma_x));
}
...
static INLINE void propagate_layer(layer_t *lower, layer_t *upper)
{
  ...
  for (nu = 0; nu < upper->no_of_neurons; nu++) {
    ...
    for (nl = 0; nl <= lower->no_of_neurons; nl++) {
      value += upper->upper_lower_weight[nu*(lower->no_of_neurons+1)+nl]
      * lower->neuron_output[nl];
    }
    upper->neuron_output[nu] = (*act) (value);
  }
}
...
float net_compute_output_error(network_t *net, const float *target)
{
  ...
  for (n = 0; n < net->output_layer->no_of_neurons; n++) {
    output = net->output_layer->neuron_output[n];
    error = target[n] - output;
    net->output_layer->neuron_error[n] = (*d_act) (output) * error;
...
static INLINE void backpropagate_layer(layer_t *lower, layer_t *upper)
{
  ...
  for (nl = 0; nl <= lower->no_of_neurons; nl++) {
    ...
    for (nu = 0; nu < upper->no_of_neurons; nu++) {
      error += upper->upper_lower_weight[nu*(lower->no_of_neurons+1)+nl]
       * upper->neuron_error[nu];
    }
    output = lower->neuron_output[nl];
    lower->neuron_error[nl] = (*d_act) (output) * error;
...
static INLINE void adjust_weights(network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      error = net->layer[l].neuron_error[nu];
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        int offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        delta =
            net->learning_rate * error * net->layer[l -
                                                    1].neuron_output[nl]
	    * net->layer[l-1].neuron_learning_rate[nl] * net->layer[l].neuron_learning_rate[nu]
            + net->momentum * net->layer[l].upper_lower_delta[offset];
        net->layer[l].upper_lower_weight[offset] += delta;
        net->layer[l].upper_lower_delta[offset] = delta;
      }
    }
  }
}
...
static INLINE void adjust_deltas_batch(network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      ...
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        int offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        net->layer[l].upper_lower_delta[offset] +=
            net->learning_rate * error * net->layer[l -
                                                    1].neuron_output[nl];
      }
    }
...
static INLINE void adjust_weights_batch(network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        int offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        net->layer[l].upper_lower_weight[offset] +=
            net->layer[l].upper_lower_delta[offset] 
	    * net->layer[l-1].neuron_learning_rate[nl] * net->layer[l].neuron_learning_rate[nu]
             / net->no_of_patterns;
      }
...
void net_jolt(network_t *net, float factor, float range)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        int offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        if (fabs(net->layer[l].upper_lower_weight[offset]) < range) {
          net->layer[l].upper_lower_weight[offset] =
              2.0f * range * ((float) random() / RAND_MAX - 0.5f);
        } else {
          net->layer[l].upper_lower_weight[offset] *=
              1.0f + 2.0f * factor * ((float) random() / RAND_MAX - 0.5f);
        }
      }
    }
  }
                ...
                float range)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
	...
	offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        offset2 = new_nu * (new_net->layer[l-1].no_of_neurons+1)+new_nl;
        new_net->layer[l].upper_lower_weight[offset2] =
            net->layer[l].upper_lower_weight[offset];
        new_net->layer[l].upper_lower_delta[offset2] =
            net->layer[l].upper_lower_delta[offset2];
      }
    }
  }
...
void net_remove_neurons(network_t *net, int layer, int neuron, int number)
{
  ...
  for (l = 1; l < new_net->no_of_layers; l++) {
	...
	offset = orig_nu * (net->layer[l-1].no_of_neurons+1)+orig_nl;
        offset2 = nu * (new_net->layer[l-1].no_of_neurons+1)+nl;
...
network_t *net_copy(const network_t *net)
{
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      for (nl = 0; nl <= net->layer[l - 1].no_of_neurons; nl++) {
        int offset = nu * (net->layer[l-1].no_of_neurons+1)+nl;
        new_net->layer[l].upper_lower_weight[offset] =
            net->layer[l].upper_lower_weight[offset];
        new_net->layer[l].upper_lower_delta[offset] =
            net->layer[l].upper_lower_delta[offset];
      }
    }
  }
  
  /* lets copy the neuron specific learning rates */
  for (l = 0; l < net->no_of_layers; l++) {
    float *lr = net->layer[l].neuron_learning_rate;
    float *lrnew = new_net->layer[l].neuron_learning_rate;
    for (nu = 0; nu < net->layer[l].no_of_neurons; nu++) {
      /* copy the original network's neuron_learning_rates */
      lrnew[nu]=lr[nu];
      }
   }
 ...
 *
 * the learning rates of the spawned neurons willbe the same as the parents.
 */
    ...
    )
{
  ...
  /* init the maps
    ...
    for (i = 0; i < net->no_of_layers; i++) {
      int s = net->layer[i].no_of_neurons +
       (int) (((float) (net->layer[i].no_of_neurons)) * ratio_to_add);     /* slightly off for input nodes, but good enough */
      if (s > maxsize)
        maxsize = s;
    }
    maxsize++;                  /* because of the bias */
    lowermap = (int *)calloc(maxsize, sizeof(int));
    uppermap = (int *)calloc(maxsize, sizeof(int));
    lowerbias = (float *)calloc(maxsize, sizeof(float));
    upperbias = (float *)calloc(maxsize, sizeof(float));
  ...
  /* handle layer 0 kearning rate */
  {int i;
  for (i = 0;i <= net->layer[0].no_of_neurons; i++) { 
    new_net->layer[0].neuron_learning_rate[i] = net->layer[0].neuron_learning_rate[i];
    }
  }
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    ...
    if (l == net->no_of_layers - 1) {   /* if we are the output layer */
      ...
      for (newpointer = 0; newpointer < new_net->layer[l].no_of_neurons;
        ...
        if (((float) (random()) / RAND_MAX) < chance) { /* we are going to do this */
	     ...
	     Yes - a node with 0 for a weight does nothing.  Also the 1/8 increments keeps the floating point numbers jiving perfectly
	     so that the child works the same as the parent HME */
	...
	new_net->layer[l].neuron_learning_rate[nu] = net->layer[l].neuron_learning_rate[oldnu];
        for (nl = 0; nl <= new_net->layer[l - 1].no_of_neurons; nl++) {
          int oldnl;
          int offset;
          int offset2;
   	  ...
   	  offset = oldnu * (net->layer[l-1].no_of_neurons+1)+oldnl;
          offset2 = nu * (new_net->layer[l-1].no_of_neurons+1)+nl;

          new_net->layer[l].upper_lower_weight[offset2] =
              net->layer[l].upper_lower_weight[offset] * lowerbias[nl];

          new_net->layer[l].upper_lower_delta[offset2] =
              net->layer[l].upper_lower_delta[offset] * lowerbias[nl];
