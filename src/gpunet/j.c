...
set_input (network_t *net, const float *input)
{
  ...
  for (n = 0; n < net->input_layer->no_of_neurons; n++) {
    neuron_t *neuron;
    neuron=net->input_layer->neuron +n;
    neuron->activation = input[n];
 ...
 * Update by Hibbard Engler October 2006
 * Instead of calling the divide function (/interval) I call the multiply function (*one_over_interval) as this is much faster.
 */
static inline float
sigma (float x)
{
...
/*
 * Update by Hibbard Engler October 2006
 * The inverse of the sigma to compute the errors correctly and avoid stuck 
 * neurons.
 */
static inline float
...
/*!\brief inverse action to better compute weights
 */
static inline float
inverse_sigma (float x)
{
 ...
 * Update by Hibbard Engler Oct 2006
 *
 * This was using array indexing alot to do the multiply.
 * I simplified this by using pointers to structures and incrementing the pointers - requiting just an add.
 * I also reduced the numebr of pointers total 
 */
static inline void
propagate_layer (layer_t *lower, layer_t *upper)
{
  float value;
  neuron_t *upper_layer_neuron;
  neuron_t *upper_layer_max_neuron;
  neuron_t *lower_layer_max_neuron;
  assert (lower != NULL);
  assert (upper != NULL);

  upper_layer_max_neuron=upper->neuron+upper->no_of_neurons;
  lower_layer_max_neuron=lower->neuron+lower->no_of_neurons;
  
  for (upper_layer_neuron=upper->neuron; 
     upper_layer_neuron < upper_layer_max_neuron; upper_layer_neuron++) {
    neuron_t *lower_layer_neuron;
    float *weight = upper_layer_neuron->weight;
    value = 0.0;
    for (lower_layer_neuron=lower->neuron; lower_layer_neuron 
    <= lower_layer_max_neuron ; lower_layer_neuron++) {
      value += *(weight++) * lower_layer_neuron->output;
    }
    upper_layer_neuron->activation = value;
    upper_layer_neuron->output = sigma (value);
  }
}
 ...
 *
 * Hibbard Engler - Oct 2006
 * The error was erroneous.  It computed the error based on the difference between
 * target and output.  When really,  the error is the difference between
 * the accumulation and the inverse sigma of the output.
 * Say that the accululation was 400 so the output would be ~1
 * but that the targer was really 0
 * 
 * Hibbard  Engler - Oct 2006 
 * I modified this routine to use pointers and increments instead of indexing an array through a pointer.  This should speed things up.
 */
float
net_compute_output_error (network_t *net, const float *target)
{
  float activation, error, output;
  neuron_t *output_layer_neuron;
  neuron_t *max_output_layer_neuron;
  assert (net != NULL);
  assert (target != NULL);

  net->global_error = 0.0;
  
  max_output_layer_neuron=net->output_layer->neuron + net->output_layer->no_of_neurons;
  
    /* the error for each neuron is not based on the 
     difference between the target and its output - no 
     it is the difference between the activation and the 
     activation amount to reach the target.
     Example.  Say we have 1000 nodes on the prior layer,  and their
     activation goes to 400.  Thus would give an output of 1. Lets also say the 
     target is 0.  
     The difference between 1 and 0 is 1.  But the real error on the weights is 
     400 - (-12) which si -12.
     
     Now if we are stubborn (i.e. dont care if things are grossly over) and the gross
     overcompensation is in the right direction -  we will not error that out.
     */  
   for (output_layer_neuron=net->output_layer->neuron; output_layer_neuron < max_output_layer_neuron; output_layer_neuron++) {
/* old:
    output = output_layer_neuron->output;
    error = *(target++) - output;
    net->global_error += error * error;
    output_layer_neuron->error = output * (1.0 - output) * error;
    */
    float target_value;
    float target_needed_activation;
    target_value = (*target++);
    output = output_layer_neuron->output;
    activation = output_layer_neuron->activation;
    target_needed_activation = inverse_sigma(target_value);
    /* compute generic error */
    error = target_value - output;
    net->global_error += error * error;
    /* now compute our back propogation error */
    if (neuron->stubborn) {
      if (activation < MIN_ENTRY) activation=MIN_ENTRY;
      if (activation > MAX_ENTRY) activation=MAX_ENTRY;
      }
    error = target_needed_activation - activation;
    }  
    output_layer_neuron->error = output * (1.0 - output) * error;
 ...
 *
 * modified by Hibbard Michael engler - Eliminated some of the indexing so the loops could run faster
 */
static inline void
backpropagate_layer (layer_t *lower, layer_t *upper)
{
  ...
  upper_layer_max_neuron = upper->neuron+upper->no_of_neurons;
  nl=0;
  for (lower_layer_neuron=lower->neuron;lower_layer_neuron <= lower_layer_max_neuron;lower_layer_neuron++) {
    error = 0.0;
    neuron_t *upper_layer_neuron;
    

    for (upper_layer_neuron=upper->neuron;upper_layer_neuron<upper_layer_max_neuron;upper_layer_neuron++) {
      error += upper_layer_neuron->weight[nl] * upper_layer_neuron->error;
    }
    output = lower_layer_neuron->output;
    lower_layer_neuron->error = output * (1.0 - output) * error;
    nl++;
  }
}
 ...
 * Hibbard Engler Oct 2006 
 * Optopmized this heavily
 * first of all, net->learning_rate never changes,  so it is better to use a local variable to hold it.
 * same with net->momentum.
 * secondly,  (net->learning_rate * error) can be precomputed
 * so net->learning_rate * error * upper_layer_output can be
 * learning_rate_times_error * upper_layer_output
 * thirdly,  momentum * net->layer[l].neuron[nu].delta[nl]
 *  well this can be momentum * (*delta++)
 * forthly,  using pointers to structurs with increments instead of multiplys
 */
static inline void
adjust_weights (network_t *net)
{
  int l;                   /* level number of the upper layer from 1 to the number of layers */
  float momentum;          /* same as net->momentum */
  float learning_rate;     /* same as net->learning_rate */
  
  assert (net != NULL);

  /* set momentum and learning late from the network_t structure */
  momentum = net->momentum;
  learning_rate = net->learning_rate;
  
  for (l = 1; l < net->no_of_layers; l++) {
    layer_t *upper_layer;              /* upper layer to work on */
    layer_t *lower_layer;              /* lower layer to work on */
    neuron_t *upper_layer_neuron;      /* the upper layer neuron that we are evaluating */
    neuron_t *upper_layer_max_neuron;  /* one higher than the largest upper layer neuron - used to bind the loop */
    neuron_t *lower_layer_max_neuron;  /* the largest lower layer neuron - also used to bind the loop */
    
    /* set the upper and lower layers */
    upper_layer=net->layer+l;
    lower_layer=upper_layer-1;

    /* compute the maximum neurons here,  before we start looping */        
    upper_layer_max_neuron = upper_layer->neuron + upper_layer->no_of_neurons;
    lower_layer_max_neuron = lower_layer->neuron + lower_layer->no_of_neurons;
    
    
    for (upper_layer_neuron=upper_layer->neuron; upper_layer_neuron<upper_layer_max_neuron;upper_layer_neuron++) { /* for each upper neuron */
      float learning_rate_times_error; /* a short cut.  error * learning rate is used later on */
      float *deltas;                  /* the array of deltas for the upper neuron */
      float *weights;                 /* the array of weights for the upper neuron */
      neuron_t *lower_layer_neuron;   /* the lower layer neuron that we will be propogating the error to */
      
      /* compute the rror of the upper neuron * the learning rate.  this is the  amount we will propogate back */
      learning_rate_times_error = upper_layer_neuron->error * learning_rate; /* saves time multiplying once here instead of once for every neuron on the lower layer */

      /*  extract the deltas array and the weights array */
      deltas = upper_layer_neuron->delta; 
      weights = upper_layer_neuron->weight;
      
      for (lower_layer_neuron=lower_layer->neuron; lower_layer_neuron <= lower_layer_max_neuron; lower_layer_neuron++) { /* for each lower neuron */
        float delta; /* the difference that we will apply proportionately to the output values */
#if 1
        delta =
          learning_rate_times_error * lower_layer_neuron->output + momentum * (*deltas);
	  /* take the previous delta value -- and use the momentum of the previous error
	  and add it to the current error * the output * the learning rate.
	  The learning rate is less than 1 so that we can handle a few bad streaks of 
	  data without redoing the entire network */
	  
        *(weights++) += delta; /* modify the current weight of this output */
        *(deltas++) = delta; /* store the change in case we have momentum */
#else /* without momentum */
        *(weights++) +=
          learning_rate_times_error *  lower_layer_neuron->output;
	/* a bit easier - learning rate limits the error learning. multiply this by the output given,  and this is the difference we wish to weigh in */
#endif
      }
    }
  }
 ...
 * \param net Pointer to a neural network.
 * Hibbard Engler Oct 2006 - I adjusted this similar to the adjust_weights routine.
 */
static inline void
adjust_deltas_batch (network_t *net)
{
  ...
  /* set learning late from the network_t structure */
  learning_rate = net->learning_rate;
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    ...
    for (upper_layer_neuron=upper_layer->neuron; upper_layer_neuron<upper_layer_max_neuron;upper_layer_neuron++) { /* for each upper neuron */
      ...
      for (lower_layer_neuron=lower_layer->neuron; lower_layer_neuron <= lower_layer_max_neuron; lower_layer_neuron++) { /* for each lower neuron */
	*(deltas++) += learning_rate_times_error *  lower_layer_neuron->output; /* take the lower levels output and adjust the errors */
      }
    }
  }
}
 ...
 * training.
 * \param net Pointer to a neural network.
 * Hibbard Engler Oct 2006 - I adjusted this similar to the adjust_weights routine for better perfomance
 */
static inline void
adjust_weights_batch (network_t *net)
{
  int l;                   /* level number of the upper layer from 1 to the number of layers */
  float one_over_no_of_patterns; /* makes it faster to multiply by the reciprocal than to convert an int into a float and divide by the float */
  
  assert (net != NULL);

  /* set one_over_no_of_patterns */
  one_over_no_of_patterns = 1. / ((float)net->no_of_patterns);
  ...
  for (l = 1; l < net->no_of_layers; l++) {
    ...
    for (upper_layer_neuron=upper_layer->neuron; upper_layer_neuron<upper_layer_max_neuron;upper_layer_neuron++) { /* for each upper neuron */
      float *deltas;                  /* the array of deltas for the upper neuron */
      float *weights;                 /* the array of weights for the upper neuron */
      float *weights_max;             /* the end of the array */
      

      /*  extract the deltas array and the weights array */
      deltas = upper_layer_neuron->delta; 
      weights_max = weights + lower_layer->no_of_neurons;
      
      for (weights = upper_layer_neuron->weight; weights <= weights_max;weights++) {
        *weights += *(deltas++) * one_over_no_of_patterns;
      }
    }
  }
}
