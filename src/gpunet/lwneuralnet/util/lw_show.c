/* lw_show.c -- show neural network computation
 * copyright (c) 2003-2005 Peter van Rossum <petervr@users.sourceforge.net>
 * $Id$ */
  ...
  while (1) {
    ...
    switch (c) {
      case 'h':
        ...
        printf("\nDescription:\n");
        printf
            ("  Show the inputs, computed outputs, targets, and error\n");
        printf
            ("  for each of the input/target pairs in the 'specification',\n");
...
#define targets(i) (target + i* no_of_outputs)
  ...
  for (i = 0; i < no_of_pairs; i++) {
    /* compute the network for inputs (i) */
    net_compute(net, inputs(i), output);

    /* find the output error for targets(i) */
    error = net_compute_output_error(net, targets(i));

    /* print everything: inputs -> outputs (targets) error */
