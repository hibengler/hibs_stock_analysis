...
/*    fprintf(stderr,"\\%s,%s,%s,%s	%f %lf\n",
  ...
  */   
    ...
    if (strcmp(dl.symbol,last_symbol)==0) { /* if it is time to do processing */
      if (c->number_points > (points_needed_for_run)) { /* If we can send a run */
        send_run(dl.dateInSeconds);
	}
      if (c->number_points > (points_needed_for_train)) { /* If we can send a train */
        send_train(dl.dateInSeconds);;
	}
...
int send_run1(char *action,double date,int offset) {
int i;
double real_date;
real_date=date;
...
for (i=0;i<number_companies;i++) {
  ...
  for (j=0;j<run_offset_length;j++) {
    ...
    volume = c->volume[(run_offsets[j]+pos+offset)&4095];
    printf("%lf,%lf,",point,volume);
    }
  }
return(0);
}
...
int send_run(double date) {
int i;
i = send_run1("run",date,0);
if (i==0) {
  printf("\n");
  }
return(i);
...
int send_train(double date) {
int i;
int return_value;
int offset;
offset = -train_offsets[train_offset_length-1];

return_value = send_run1("train",date,offset);
...
for (i=0;i<number_companies;i++) {
  ...
  for (j=0;j<train_offset_length;j++) {
    double point;
    double volume;
    point = c->point[(train_offsets[j]+pos+offset)&4095];
    volume = c->volume[(train_offsets[j]+pos+offset)&4095];
    printf("%lf,%lf,",point,volume);
    }
  }
printf("\n");
