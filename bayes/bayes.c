...

  {
    ...
    while (fgets(line2,19999,pf) != NULL)
    {
      ...
      if (line2[0]=='*')
      {
        ...
        {
	  line2[strlen(line2)-1] = '\0'; /* strip off the carriage return */
	  fprintf(stderr,"line is '%s'\n",line2+2);
          time = str_to_date(line2+2);
	  fprintf(stderr,"predict %lf\n",time);
          base_time =	    time;
              ...
              /* input is set up in theory we have output set to what it will be.  Lets compute predict */
              {
			    ...
			    dist=1;
			      ...
			      int zz=z;
                              if (z -dist >=0 ) {
			         z = z - dist;
  			         ff = ff - (ff % positions) + z;
				 
       	/*		    fprintf(stderr,"vchange %d to %d (pos %d to %d)\n"
			    , f,ff,(f % positions) ,z);*/
                                 if (sum_inputs[ff]) break;
				 }
			      z=zz;
			      if (z + dist < positions)  {
			         z = z + dist;
  			         ff = ff - (ff % positions) + z;
/*       fprintf(stderr,"^change %d to %d (pos %d to %d)\n", f,ff,(f % positions) ,z);*/
                                 if (sum_inputs[ff]) break;
				 }
			      z=zz;
			      if ((z -dist <0 )&& (z + dist >= positions))  {
/*                                fprintf(stderr,"input sum not there wtf?\n");*/
			      ...
			      dist++;
                            ...
                            {
                              if (sum_all[ff])
                                predict[kpl] += log((double)(sum_all[ff])) - log(((double)(sum_inputs[ff])));
                            }
		      ...
		      int q;
                      predict[i] *= sum;
                      q= (((int)(predict[i]*100.f)));
		      if (q && (q > 49) && (i-(k*positions) != positions/2))
	  ...
	  line2[strlen(line2)-1] = '\0'; /* strip off the carriage return */
          time = str_to_date(line2+2);
          fprintf(stderr,"would verify here %lf\n",time);
...

          {
            ...
            for (k=0;k<number_of_companies;k++)
            {
              ...
              m=0;
              {
                ...
                in2 = price_at_time(y,time-900.*point_offsets[m]);
              	fprintf(stderr,"in=2 %s %lf is %f\n",y->symbol,time-900.*point_offsets[m],in2);
                ...
                respos =  ((int)(res * ((double)(positions))) );
                if (respos==positions) respos--;
                   	  fprintf(stderr,"   %d in %s %lf is %f    res %f  respos %d predict %d precent\n"
			  ,m,y->symbol,time-900.*point_offsets[m],in2,res,respos,((int)(100.f*predict[k*positions+respos])));
